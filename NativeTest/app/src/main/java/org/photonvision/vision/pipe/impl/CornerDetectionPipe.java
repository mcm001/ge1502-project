...
public class CornerDetectionPipe
    ...
    */
    private List<Point> findBoundingBoxCorners(TrackedTarget target) {
        ...
        List<Point> list_ = Arrays.asList(points);
        list_.sort(leftRightComparator);
        // of this, we now have left and right
        // sort to get top and bottom
        ArrayList<Point> left = new ArrayList<>(Arrays.asList(list_.get(0), list_.get(1)));
        left.sort(verticalComparator);
        ArrayList<Point> right = new ArrayList<>(Arrays.asList(list_.get(2), list_.get(3)));
        right.sort(verticalComparator);

        // tl tr bl br
        Point tl = left.get(0);
        Point bl = left.get(1);
        Point tr = right.get(0);
        Point br = right.get(1);
    ...
    */
    private List<Point> detectExtremeCornersByApproxPolyDp(TrackedTarget target, boolean convexHull) {
        Point centroid = target.getMinAreaRect().center;
        ...
        */
        boolean isOpen = !convexHull && target.hasSubContours();
        double peri = Imgproc.arcLength(targetContour, true);
        ...
        ArrayList<Point> pointList = new ArrayList<>(polyOutput.toList());
        ...
        List<Point> boundingBoxCorners = findBoundingBoxCorners(target);

        Comparator<Point> distanceToTlComparator =
                Comparator.comparingDouble((Point p) -> distanceBetween(p, boundingBoxCorners.get(0)));

        Comparator<Point> distanceToTrComparator =
        ...
        Point bl = leftList.get(leftList.size() - 1);
        Point br = rightList.get(rightList.size() - 1);
        return Arrays.asList(tl, bl, br, tr);
    }
