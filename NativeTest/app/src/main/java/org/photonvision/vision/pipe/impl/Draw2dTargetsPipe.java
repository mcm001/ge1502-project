...
public class Draw2dTargetsPipe
    ...
    protected Void process(Pair<Mat, List<TrackedTarget>> in) {
        double imageSize = Math.sqrt(in.getLeft().rows() * in.getLeft().cols());
        double textSize = params.kPixelsToText * imageSize;
        double thickness = params.kPixelsToThickness * imageSize;
        ...
        if (!in.getRight().isEmpty()
            ...
            Scalar centroidColour = ColorHelper.colorToScalar(params.centroidColor);
            Scalar maximumBoxColour = ColorHelper.colorToScalar(params.maximumBoxColor);
            Scalar rotatedBoxColour = ColorHelper.colorToScalar(params.rotatedBoxColor);
            Scalar circleColor = ColorHelper.colorToScalar(params.circleColor);
            Scalar shapeColour = ColorHelper.colorToScalar(params.shapeOutlineColour);
            ...
            for (int i = 0; i < (params.showMultipleTargets ? in.getRight().size() : 1); i++) {
                ...
                if (params.shouldShowRotatedBox(target.getShape())) {
                    ...
                    if (poly != null) {
                        ...
                        Imgproc.drawContours(
                                in.getLeft(), Arrays.asList(mat), -1, ColorHelper.colorToScalar(Color.BLUE), 2);
                        mat.release();
                    }
                }
                ...
                if (params.showContourNumber) {
                    Point center = target.m_mainContour.getCenterPoint();
                    Point textPos =
                            new Point(
                ...
                if (params.showCentroid) {
                    ...
                    int crosshairRadius = (int) (imageSize * params.kPixelsToCentroidRadius);
                    double x = centroid.x;
                    double y = centroid.y;
    ...
    public static class Draw2dTargetsParams {
        ...
        public int centroidColor = Color.GREEN; // Color.decode("#ff5ebf");
        public int rotatedBoxColor = Color.BLUE;
        public int maximumBoxColor = Color.RED;
        public int shapeOutlineColour = Color.MAGENTA;
        public int textColor = Color.GREEN;
